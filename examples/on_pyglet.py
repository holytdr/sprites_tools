...
def apply_actor(sprites, frame):
    ...
    return ImageData(sprite.width, sprite.height, 'RGBA', b''.join(sprite.data))


def reverse(sprite):
    rdata = []
    for i in range(sprite.height - 1, -1, -1):
...
actor = act_open(os.path.join(data_dir, 'wolf.act'))
sprites = spr_open(os.path.join(data_dir, 'wolf.spr'))
