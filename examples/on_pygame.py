...
if __name__ == '__main__':
    ...
    wolfs = setup_animations(act_open(os.path.join(data_dir, 'poring.act')),
                             spr_open(os.path.join(data_dir, 'poring.spr')))
    print(len(wolfs))
    wolfs = [x for i, x in enumerate(wolfs) if i % 4 == 2]
    y = 20
    for i in wolfs:
        i.x = 50
        i.y = y
    ...
    lunatics = setup_animations(act_open(os.path.join(data_dir, 'lunatic.act')),
                                spr_open(os.path.join(data_dir, 'lunatic.spr')))
    ...
    green_plant = setup_animations(act_open(os.path.join(data_dir, 'green_plant.act')),
                                   spr_open(os.path.join(data_dir, 'green_plant.spr')))
