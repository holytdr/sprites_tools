...
class Frames(list):
    ...
    def fill_rgba(self, frame):
        palette = self.palette[0:1] + [i[:3] + bytearray((255,)) for i in self.palette[1:]]
...
def spr_open(name):
    ...
    # read palette
    frames.palette = [f.read(4) for _ in range(256)]
...
if __name__ == '__main__':
    name = 'wolf'
    file_path = r'E:\GameProjects\pyro-tools\examples\monsters\{}.spr'.format(name)
    frames = spr_open(file_path)
    for i, frame in enumerate(frames, 1):
        frames.fill_rgba(frame)
        im = Image.frombytes('RGBA', (frame.width, frame.height), b''.join(frame.data))
        fn = r'E:\GameProjects\sprite_test\{}-{}.png'.format(name, i)
        im.save(fn, 'PNG')
        print(fn)
