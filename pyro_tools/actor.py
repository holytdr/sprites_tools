...
def act_open(file_path):
    ...
    # read actions
    for i_a in range(animations_count):
        frames_count = to_int()

        animation = Animation()
        for i_f in range(frames_count):
            # skip trash
            f.seek(f.tell() + 32)

            subframes_count = to_int()
            subframes = []
            for i_sf in range(subframes_count):
    ...
    # sound
    n_sound_file = to_int()
    for i in range(n_sound_file):
        sound_file_name = f.read(40).decode('ascii').strip('\x00')
...
#             print sound
if __name__ == '__main__':
    file_path = r'E:\GameProjects\pyro-tools\examples\monsters\poring.act'
    actor = act_open(file_path)
    attrs = []
    for animation in actor.animations:
        attr = {
            'image_n': [], 'rotation': [], 'direction': [], 'offset_x': [], 'offset_y': [], 'scale_x': [], 'scale_y': []
        }
        ...
        attrs.append(attr)
        print("frames: {}, rotation: {}, speed: {:.2f}".format(attr['image_n'], attr['rotation'], attr['speed']))
    for sound in actor.sounds:
        print(sound)
